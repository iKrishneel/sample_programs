                          ...
                          const float j_thresh = 0.450f)
{
   ...
   for (int i = 0; i < hcurvature_pts.size(); i++) {
      ...
      if(weight > j_thresh)
      {
         ...
         circle(image, cur_pt, 3, Scalar(255,0,0), -1);
      }
   }
                               ...
                               std::vector<EdgeParam> &end_points)
{
   ...
   for (int j = 0; j < junction_points.size(); j++) {
      cv::circle(image, junction_points[j].index, 1, Scalar(0,255,0), CV_FILLED);
      
      for (int y = -1; y <= 1; y++) {
         for (int x = -1; x <= 1; x++) {
            cv::Point pt = cv::Point(junction_points[j].index.x + x, junction_points[j].index.y + y);
            std::cout << (float)img_bw.at<uchar>(pt.y, pt.x) << "\t";
         }
         std::cout << std::endl;
      }
      std::cout << std::endl;
      imshow("movements", image);
      waitKey(0);
   }
   imshow("movements", image);
}
                          ...
                          std::vector<EdgeParam> &hcurvature_pts)
{
   ...
   traverseJunction2EndPoint(img_bw, junction_pts, end_points);
