                          ...
                          std::vector<EdgeParam> &hcurvature_pts)
{
   ...
   cv::flann::KDTreeIndexParams indexParams(neighbor_size);

   cv::Mat featureMD = cv::Mat(static_cast<int>(hcurvature_pts.size()), sizeof(short), CV_32F);
   for (int j = 0; j < hcurvature_pts.size(); j++) {
      featureMD.at<float>(j,0) = hcurvature_pts.at(j).index.x;
      featureMD.at<float>(j,1) = hcurvature_pts.at(j).index.y;
   }
   cv::flann::Index kdTree = cv::flann::Index(featureMD, indexParams);

   for (int i = 0; i < hcurvature_pts.size(); i++) {
      cv::Mat query = cv::Mat(sizeof(char), sizeof(short), CV_32F);
      query.at<float>(0,0) = hcurvature_pts[i].index.x;
      query.at<float>(0,1) = hcurvature_pts[i].index.y;

      std::cout << query << std::endl;
      Mat indices;
      Mat distances;
      kdTree.knnSearch(query, indices, distances, neighbor_size, cv::flann::SearchParams(4));

      // filter the those indices that have been assigned
      for (int y = 0; y < indices.cols; y++) {
         circle(image, hcurvature_pts[static_cast<int>(indices.at<int>(0,y))].index, 3, Scalar(0,255,0), -1);
         hcurvature_pts.erase(hcurvature_pts.begin() + static_cast<int>(indices.at<int>(0,y)));
      }
      hcurvature_pts.erase(hcurvature_pts.begin() + i);
      std::cout << "Vector Size: " << hcurvature_pts.size() << std::endl;

      imshow("knn search", image);
      waitKey(0);
   }
}
                          ...
                          std::vector<EdgeParam> &hcurvature_pts)
{
   ...
   filterJunctionPoints(img_bw, junction_points);
